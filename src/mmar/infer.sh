...
if [ -z "${result_dir}" ]; then
    result_dir="${MAPO_ROOT}/exp/mmar/results/Qwen3-Omni-30B-A3B-Thinking"
fi

if [ -z "${vllm_tensor_parallel_size}" ]; then
    IFS=',' read -r -a _mmar_devices <<<"${devices}"
    vllm_tensor_parallel_size=${#_mmar_devices[@]}
fi
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    ...
    VLLM_PORT=8001 \
        ...
        swift infer \
        --model ${model_name_or_path} \
        --val_dataset ${MAPO_ROOT}/data/raw/mmar/processed-mmar.jsonl \
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
    CUDA_VISIBLE_DEVICES=${devices} \
        ...
        -r ${MAPO_ROOT}/data/raw/mmar/MMAR-meta.json \
