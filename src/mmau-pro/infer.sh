...
if [ -z "${vllm_tensor_parallel_size}" ]; then
    IFS=',' read -r -a _mmau_pro_devices <<<"${devices}"
    vllm_tensor_parallel_size=${#_mmau_pro_devices[@]}
fi
...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
    ...
    ENABLE_AUDIO_OUTPUT=0 \
        ...
        swift infer \
        --model ${model_name_or_path} \
        --val_dataset ${MAPO_ROOT}/data/raw/mmau-pro/processed-mmau-pro.jsonl \
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
    CUDA_VISIBLE_DEVICES=${devices} \
        ...
        -r ${MAPO_ROOT}/data/raw/mmau-pro/mmau-pro.json \
    ...
    CUDA_VISIBLE_DEVICES=${devices} \
        python "${script_dir}/scripts/json2parquet.py" \
        -i ${result_dir}/qwen3-omni${suffix}.normalized.jsonl \
        -o ${result_dir}/qwen3-omni${suffix}.normalized.parquet
fi
